...
class screenMain(Serial_SQL_Logger_GUI.FenetrePrincipaleClass):
	...
	def __init__(self,parent):
		...
		APP_TITRE = "Commandes série vers Afficheur Arduino Uno"
	...
	def m_COMmanuMnuEvt(self,event):
		...
		# scan port serie dispo:
		print("<Select.Manuel> ")
		box = wx.TextEntryDialog(None, "Entrez le nom du port serie:\n  Exemple: COM1", "Port serie:", "COM1")
		if box.ShowModal()==wx.ID_OK:
			COMselectGbl=box.GetValue()
		#screenSaisieCOM.saisieComDlgClass.ShowModal()
		#screenSaisieCOM.saisieComDlgClass.Show(True)
		#COMselectGbl = COMmanuTextCtrl.Value
	...
	# capture de l'événement clic 'Ok'
	def m_transfertBtnEvt(self,event):
		global COMselectGbl
		global COMvitesseGbl
		print("Capture evenement bouton OK")
		...
		# la variable est definie, on continue
			...
			try:
				uno = serial.Serial(str(COMselectGbl),int(COMvitesseGbl))
				#Temps d'attente d'initialisation du port série
				print("Attente d'initialisation du port serie")
				sleep(0.5)
				chaine = self.commandeTextCtrl.Value
				print(chaine)
				self.m_progressBar.Range = len(chaine)
				progressBar = 1
				# boucle permettant d'envoyer les chiffres vers le port série
				for chiffre in chaine:
					# Ecriture du chiffre pointé vers le port série
					uno.write(str(chiffre))
					# debuggage
					# print(progressBar)
					# actualisation de la barre de progression
					self.m_progressBar.Value = progressBar
					progressBar = progressBar +1
					# Attente de x secondes en fonction du slider de vitesse
					sleep(self.m_vitesseSld.Value / 1000)
...
class screenSaisieCOM(Serial_SQL_Logger_GUI.saisieComDlgClass):
	# constructor
	def __init__(self,parent):
		# initiaslise la class parent
		Serial_SQL_Logger_GUI.saisieComDlgClass.__init__(self,parent)
